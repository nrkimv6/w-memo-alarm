...
> **진행 상태**: Phase 1 (P0) 구현 완료 (2026-02-04)
> - STEP 1~13: 타입 정의, Store, SW 기록, 포그라운드 기록, Capacitor 기록, 페이지 UI, 하단 네비 추가 완료
> - STEP 14 (테스트): 수동 테스트 필요
> - STEP 15 (Phase 2): 미착수
...
15-1. 여기까지 완료되면 Phase 1이 끝난 것이다. Phase 2의 필터링, 삭제, Supabase 동기화는 별도 작업으로 진행한다.

15-2. Supabase 테이블 마이그레이션 SQL 파일은 data/migrations/ 폴더에 005번으로 추가한다. 기존 004_supabase_online_first.sql 파일의 테이블 생성 패턴(ma_ 접두사, RLS 정책)을 참고한다.

15-3. 통계 요약 화면은 알림 내역 페이지 상단에 접기/펼치기 가능한 섹션으로 추가하는 것을 권장한다.
