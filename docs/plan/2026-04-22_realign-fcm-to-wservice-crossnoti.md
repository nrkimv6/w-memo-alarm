...
| 우선순위 | 항목 | 설명 | 난이도 |
|:-------:|------|------|:------:|
| P0 | 서버 secret 원복 | `send-notifications`가 provided JSON과 project id `wservice-cross-noti`를 기준으로 발송하도록 정렬 | 중간 |
...
- 클라이언트 전환에는 `apiKey`, `authDomain`, `storageBucket`, `messagingSenderId`, `appId`, `VAPID key` 7종이 필요하며 **2026-04-23 전량 수집 완료** (실값: obsidian `2026-02-11_key.md` + todo-2 Phase 3.1 체크박스 아래 템플릿).
- `memo-alarm/.env`는 `common/.env.shared` 심볼릭 링크로 `line-minder` 등 다른 wtools 앱과 Firebase env를 공유한다. 본 plan은 shared 파일을 건드리지 않고 memo-alarm 전용 `.env.local` override로만 처리한다 (SvelteKit dotenv 우선순위 `.env.local` > `.env`). override로 해결 불가능이 확인되는 경우에만 shared 분리로 fallback.
