...
- [x] 상기 개별 추가 UI
  - "앱 기본 시간 사용" 토글 (useGlobalRemind)
  - 토글 off 시 개별 상기 시각 0~N개 추가: 프리셋 버튼 + [직접 시간 지정하기] → 시각 또는 "기한 N분 전"
  - 사용자 친화적 용어 사용: "매일 리마인더 - 잊지 않도록 매일 정해진 시간에 알려드려요"
- [x] 알람 개별 추가 UI
  - 자동알람 사용 여부 토글 (useGlobalAutoAlert)
  - 수동 알람 0~N개 추가: 프리셋 버튼 + [직접 알람 추가하기] → 시각 입력 또는 특정 날짜/시간
  - 사용자 친화적 용어 사용: "마감 알람 - 기한이 다가올 때 강하게 알려드려요 (진동/소리)"
