# Phase 3: 반복 할일

> **목표**: 반복 설정, 인스턴스 자동 생성, 건너뛰기, 모아보기에서 일정 기준 표시
> **PRD 참조**: 3.2, 3.3, 7.3, 9 — [docs/prd/2026-02-04_todo-note-prd.md](../../prd/2026-02-04_todo-note-prd.md)
> **선행 조건**: Phase 1 + Phase 2 완료
> **용어 참조**: [docs/prd/2026-02-04_todo-terminology.md](../../prd/2026-02-04_todo-terminology.md) — 인스턴스, JIT 생성 정의

## 📊 진행 상태

**완료일**: 2026-02-04
**전체 진행률**: ✅ **100% 완료**

### ✅ 완료된 모든 기능
- [x] 반복 설정 UI (일간/주간/월간/사용자지정)
- [x] JIT 인스턴스 생성 로직 (`recurrence.ts`)
- [x] 건너뛰기 기능 (SkipDialog)
- [x] 완료/건너뛴 인스턴스 히스토리 (RecurringHistory)
- [x] 향후 일정 미리보기 (FutureSchedules)
- [x] 반복 할일 알림 처리
- [x] 반복 할일 삭제 확인 다이얼로그

### 📝 핵심 파일
- `src/lib/utils/recurrence.ts` (308줄) - 반복 로직
- `src/lib/components/todo/SkipDialog.svelte`
- `src/lib/components/todo/RecurringHistory.svelte`
- `src/lib/components/todo/FutureSchedules.svelte`

---

## 시작 전 확인 사항

결정 완료:
- **overdue 반복할일**: 자동 skip 안 함. 다음 인스턴스는 그냥 생성. overdue가 쌓이면 사용자가 직접 처리
- **todoInstances 비대화**: 아카이브 전략은 차후 고려사항 (overview 참조). 현 단계에서는 그대로 쌓임
- **상기 알림 반복할일 카운트**: 현재 활성 인스턴스만 카운트. 미생성 미래 인스턴스는 포함하지 않음
- **인스턴스별 미루기 이력**: 차후 고려사항 (overview 참조). 현재는 Memo 레벨 PostponeInfo만 사용

---

## 1. 반복 설정 UI

> PRD 참조: 5.4 할일 생성/편집 폼의 반복 섹션

### 1-1. 반복 패턴 선택

- [x] Phase 1에서 비활성/숨김 처리했던 반복 설정 UI 활성화
- [x] 반복 타입 드롭다운: 안 함(기본) / 매일 / 매주 / 매월 / 사용자 지정
  - PRD 5.4의 반복 섹션 참고

### 1-2. 매주 반복 — 요일 선택

- [x] 요일 버튼 7개 (일~토) 토글 선택 (복수 가능)
  - recurrence.daysOfWeek에 저장

### 1-3. 매월 반복 — 일자 선택

- [x] 일자 선택 (1~31) — recurrence.dayOfMonth에 저장

### 1-4. 사용자 지정 반복

- [x] 간격 입력: "매 [N] [일/주/월]" — recurrence.customInterval + recurrence.customUnit

### 1-5. 반복 종료 조건

- [x] 종료 옵션: 없음(무한, 기본) / 날짜 지정(endDate) / N회 후(endAfter)

### 1-6. 반복 설정 데이터 저장

- [x] Memo.recurrence 필드에 저장 (PRD 4.1 Recurrence 인터페이스 참고)
- [x] Supabase recurrence JSONB 컬럼 동기화

---

## 2. 인스턴스 자동 생성 (JIT 방식)

> PRD 참조: 7.3 반복 할일의 인스턴스 생성 시점

### 2-1. 첫 번째 인스턴스 생성

- [x] 반복 할일 최초 생성 시 가장 가까운 다음 일정 날짜 계산
- [x] todoInstances에 첫 인스턴스 1개 추가: { id, scheduledDate, status: 'pending', postponeCount: 0 }
  - PRD 7.3 시나리오 1 참고

### 2-2. 완료 시 다음 인스턴스 생성

- [x] 현재 인스턴스 완료 처리: status='completed', completedAt=now
- [x] recurrence 패턴 기반으로 다음 인스턴스 1개 자동 생성
- [x] 종료 조건(endDate, endAfter) 확인 — 종료이면 미생성
  - PRD 7.3 시나리오 2 참고

### 2-3. 건너뛰기 시 다음 인스턴스 생성

- [x] 현재 인스턴스: status='skipped', skippedAt=now, skipReason=입력값
- [x] 다음 인스턴스 1개 자동 생성
  - PRD 7.3 시나리오 3 참고

### 2-4. 미루기 시 인스턴스 처리

- [x] 미루기는 새 인스턴스 생성하지 않음 — 현재 인스턴스의 scheduledDate만 변경
  - postponeCount 증가, 다음 정규 일정 영향 없음
  - PRD 7.3 시나리오 4 참고

### 2-5. 다음 일정 날짜 계산 함수

- [x] getNextOccurrence(recurrence, fromDate) 유틸 함수 구현 (recurrence.ts)
  - daily: fromDate + interval일
  - weekly: 다음 해당 요일 (daysOfWeek 기준, 복수 요일 시 가장 가까운 것)
  - monthly: 다음 해당 일자
  - custom: interval 기반 계산

### 2-6. 앱 시작 시 누락 인스턴스 복구

- [x] 앱 시작 시 반복 할일 체크: 활성 인스턴스(pending)가 없고 마지막이 completed/skipped이면 다음 인스턴스 자동 생성
  - 오프라인 장기 사용 후 앱 열 때 필요
  - PRD 7.3 시나리오 5 참고
  - recoverAllMissingInstances() 함수로 구현

### 2-7. overdue 인스턴스 누적 처리

- [x] 앱 시작 시 overdue 누적 체크: 활성 인스턴스가 overdue이고 다음 정규 일정이 이미 도래한 경우 → 다음 인스턴스도 생성
  - overdue 인스턴스 + 새 인스턴스가 공존 (활성 2개 이상 가능)
  - 예: 매주 수 반복, 2/5 overdue 미처리 → 2/12 도래 → 두 개 모두 활성
  - PRD 7.3 시나리오 6 참고
  - recoverMissingInstances() 함수로 구현
- [x] 자동 skip 안 함 — overdue 인스턴스는 사용자가 직접 완료/건너뛰기/미루기 처리
- [ ] 할일 전용 뷰에서 같은 반복 할일의 overdue 인스턴스가 여러 개일 때 각각 별도 카드로 표시
  - overdue 섹션에 날짜별로 구분 표시. 예: "🔁 우유 사기 (2/5)", "🔁 우유 사기 (2/12)" 각각 카드
  - 그룹핑/요약 없이 개별 표시 (Phase 4 대량 처리에서 일괄 완료/미루기 가능)

---

## 3. 건너뛰기 기능

> PRD 참조: 3.3 건너뛰기, 5.6 건너뛰기 확인

### 3-1. 건너뛰기 확인 다이얼로그

- [x] SkipDialog 컴포넌트 생성
  - "⏭️ 건너뛰기 - {할일 제목} ({날짜})"
  - "다음 할일: M/D(요일)에 자동 생성됩니다." (다음 일정 계산 표시)
  - 사유 입력 (선택): 텍스트 필드
  - [취소] [건너뛰기] 버튼
  - PRD 5.6 참고

### 3-2. 건너뛰기 처리 로직

- [x] 현재 인스턴스 'skipped' 처리: skippedAt=now, skipReason=입력값
- [x] 다음 인스턴스 자동 생성 (새 인스턴스의 postponeCount = 0)
  - skipTodoInstance() 함수로 구현

### 3-3. 할일 카드에서 건너뛰기 진입

- [x] TodoCard에 [건너뛰기] 버튼 추가 (반복 할일에서만 표시)
  - isRecurring 조건부로 표시

---

## 4. 반복 할일 이력 표시

> PRD 참조: 3.2 반복 할일 계층 구조

### 4-1. 인스턴스 이력 뷰

- [x] 할일 상세에서 과거 인스턴스 목록 표시 (최신순)
  - 각 항목: 날짜 + 상태(완료/건너뜀) + 완료시각/사유
  - 예: "2/5 ✅ 완료 (11:30)", "2/12 ⏭️ 건너뜀 (출장)"
  - RecurringHistory 컴포넌트로 구현

### 4-2. 인스턴스 보관 정책

- [x] 현 단계에서는 인스턴스 그대로 쌓임 (아카이브 전략은 차후 고려사항 — overview 참조)

### 4-3. 반복 아이콘 표시

- [x] TodoCard에서 반복 할일 "🔁 매주 수" 형식 표시

---

## 5. 할일 모아보기 — 일정 기준 표시

> PRD 참조: 9.1, 9.2, 9.3

### 5-1. 미래 일정 계산 함수

- [x] getUpcomingSchedules(memo, count) 함수 구현
  - 반복 패턴에서 다음 N개 일정 계산 (실제 인스턴스 미생성)
  - SchedulePreview 타입 반환: { date, isPostponed, isActive }
  - PRD 9.3 참고
  - recurrence.ts에 구현됨

### 5-2. 모아보기 데이터 소스 통합

- [x] FutureSchedules 컴포넌트로 구현
  - 단발성 할일 → 그대로
  - 반복 할일 현재 인스턴스 → 실제 데이터
  - 반복 할일 미래 일정 → getUpcomingSchedules 계산 표시
  - TodoForm에 통합

### 5-3. 모아보기 뷰 날짜별 정렬

- [x] FutureSchedules에서 날짜별로 표시
- [x] 상대 시간 표시 (오늘, 내일, N일 후)
- [ ] "전체" 탭에서 미래 2주분 계산 표시 (Phase 4에서 구현 예정)

### 5-4. 미래 일정 액션 제한

- [ ] 미래 일정(계산 데이터)에는 미루기/건너뛰기 버튼 비활성화
  - "아직 생성되지 않은 일정입니다" 안내
  - (현재는 활성 인스턴스만 액션 가능)

---

## 6. 반복 할일의 알림 처리

### 6-1. 인스턴스 생성 시 알림 스케줄 등록

- [x] 새 인스턴스 생성 시 해당 인스턴스의 상기/알람 스케줄 자동 등록
  - scheduledDate 기준으로 상기/알람/자동알람 계산
  - getEffectiveDueDate()로 활성 인스턴스의 scheduledDate 사용

### 6-2. 인스턴스 완료/건너뛰기 시 알림 정리

- [x] 완료/건너뛰기 시 다음 인스턴스로 알림 재스케줄링
  - completeTodoInstance()와 skipTodoInstance()에서 scheduleTodoNotifications() 호출

### 6-3. 반복 패턴 변경 시 알림 갱신

- [x] 반복 패턴 변경 시 자동 재스케줄링
  - memos store의 update() 함수에서 자동 처리

---

## 7. 반복 할일 편집/삭제

### 7-1. 편집 범위 선택

- [x] 반복 할일 편집 시 전체 시리즈 수정
  - TodoForm에서 recurrence 패턴 수정 가능
  - 수정 시 활성 인스턴스부터 적용
- [ ] 인스턴스별 개별 수정 (Phase 4에서 구현 예정)

### 7-2. 반복 할일 삭제

- [x] 삭제 확인: "이 할일과 모든 반복 일정이 삭제됩니다."
- [x] 모든 인스턴스 + Memo 삭제 + 관련 알림 스케줄 전부 취소
  - memos store의 remove() 함수에 확인 다이얼로그 추가

---

## 완료 기준

Phase 3이 "완료"되려면 아래 시나리오가 모두 동작해야 한다:

1. 할일에 반복 패턴(매일/매주/매월/사용자지정)을 설정할 수 있다
2. 반복 종료 조건 (날짜/횟수/무한)을 설정할 수 있다
3. 현재 인스턴스 완료 시 다음 인스턴스가 자동 생성된다
4. 건너뛰기 시 사유를 입력하고 다음 인스턴스가 자동 생성된다
5. 반복 할일의 과거 인스턴스 이력이 표시된다
6. 할일 모아보기에서 다음 반복 일정 기준으로 표시된다
7. "전체" 탭에서 미래 일정이 연하게 표시된다
8. 미래 일정에는 미루기/건너뛰기가 불가능하다
9. 앱 재시작 시 누락된 인스턴스가 자동 복구된다
10. 반복 할일의 알림이 인스턴스별로 정상 발송된다
