...
### ✅ 완료된 모든 기능
- [x] 데이터 모델 확장 (타입 정의, Supabase 마이그레이션)
- [x] 할일 CRUD (생성, 조회, 수정, 삭제, 완료 토글)
- [x] 할일 전용 뷰 (`/todos` 페이지)
- [x] 설정 페이지 (`/settings` 내 할일 기본값)
- [x] localStorage 스키마 (`todoDefaults`)
- [x] 모든 체크박스 항목 완료
...
- [x] TodoCard 컴포넌트 생성 (별도 컴포넌트로 분리)
  - 왼쪽 체크박스 + 제목
  - 우선순위 배지 (긴급=빨강, 높음=주황, 보통=파랑, 낮음=회색)
  - PRD 5.2의 카드 레이아웃 참고
- [x] 기한 표시: "📅 2/5(수) 18:00" 형식
- [x] 알림 요약 표시: todoTiming에서 읽어 표시 (Phase 2에서 활성화)
...
- [x] `src/routes/settings/+page.svelte`에 "할일 기본설정" 섹션 추가
  - PRD 6.1의 UI 와이어프레임 참고
- [x] 상기 알림 ON/OFF 토글 + 시각 선택 (기본 07:00)
  - 설명: "매일 이 시각에 할일 상기 알림을 보냅니다. (기한 없는 할일도 포함)"
- [x] 자동 알람 ON/OFF 토글 (기본 OFF) + 기한 전 프리셋 (30분/1시간/3시간/하루/3일/1주)
- [x] 기한 초과 표시 ON/OFF 토글 (기본 ON)
- [x] 완료 통계 표시 ON/OFF 토글 (기본 ON)
- [x] 적용 중인 할일 건수 표시: "현재 N개의 할일이 있습니다"

### 6-3. 설정 ↔ Todo 연동

- [x] 설정 변경 시 useGlobalRemind=true인 할일 상기 시각 일괄 업데이트 (settingsStore에 메서드 구현)
- [x] 설정 변경 시 useGlobalAutoAlert=true인 할일 자동알람 일괄 업데이트 (settingsStore에 메서드 구현)
  - PRD 6.3의 설정 연동 흐름 참고
- [x] 새 할일 생성 시 앱 설정에서 기본값 상속
