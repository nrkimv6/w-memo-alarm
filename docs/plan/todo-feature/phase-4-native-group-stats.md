...
**전체 진행률**: 60% (웹 기능 100% 완료, Native 기능 0%)

### ✅ 완료된 기능 (Web-based)
- [x] Section 4-1: 태그/폴더 기반 필터링
- [x] Section 5: 일간/주간 진행률 표시
- [x] Section 6: 완료 통계 (기본 통계, 스트릭, 미루기 통계, 통계 페이지)
- [x] Section 7: 대량 처리 (다중 선택, 일괄 완료/삭제, overdue 빠른 액션)
- [x] Section 8-1, 8-2: 메모 ↔ 할일 전환

### 🚧 Native 기능 (Capacitor 플러그인 개발 필요)
- [ ] Section 1: Native Fullscreen Alert (Android)
...
- [x] 할일 전용 뷰에서 태그별 필터 강화 — 태그 클릭 시 해당 태그 할일만 표시
- [x] 폴더별 할일 표시 — 폴더 내 할일만 필터링
- [x] 할일 생성/수정 폼에서 폴더 선택 — 기존 폴더 시스템 활용 드롭다운 셀렉터
...
- [x] 일간/주간/월간 완료율 계산 및 표시

### 6-2. 스트릭 (연속 완료)

- [x] 연속 완료 일수 계산: 매일 할일을 모두 완료한 연속 일수 표시 ("🔥 N일 연속 완료!")

### 6-3. 미루기 통계

- [x] 가장 많이 미룬 할일 Top 3 (postponeInfo.count 기준)

### 6-4. 통계 페이지/섹션

- [x] 할일 전용 뷰에 통계 탭/섹션 추가 (또는 설정 > 통계 페이지)
...
- [x] 할일 목록에서 길게 누르기 → 다중 선택 모드 활성화 (버튼 방식으로 구현)
- [x] 체크박스로 여러 할일 선택

### 7-2. 일괄 액션

- [x] 일괄 완료: 선택한 할일 모두 'completed' 변경
- [ ] 일괄 미루기: 선택한 할일 모두 같은 날짜로 미루기 (PostponeSheet 활용)
- [x] 일괄 삭제: 선택한 할일 모두 삭제 (확인 다이얼로그)

### 7-3. overdue 일괄 처리

- [x] overdue 섹션에 "모두 내일로 미루기" 또는 "모두 완료" 빠른 버튼
...
### ✅ Phase 4-A (Web 기능) - **완료됨**

다음 시나리오가 모두 동작한다:

1. ✅ 태그/폴더로 할일을 필터링하여 볼 수 있다
2. ✅ 일간/주간 진행률이 듀얼 프로그레스 바로 표시된다
3. ✅ 완료 통계 (스트릭, 일/주/월 완료율, 미루기 Top 3)가 표시된다
4. ✅ 여러 할일을 선택하여 일괄 완료/삭제할 수 있다
5. ✅ Overdue 섹션에서 빠른 액션 (모두 선택, 내일로 미루기, 모두 완료)을 사용할 수 있다
6. ✅ 메모 ↔ 할일 전환이 가능하다

### 🚧 Phase 4-B (Native 기능) - **미구현** (Capacitor 플러그인 개발 필요)
