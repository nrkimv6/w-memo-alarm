...
구현 단계에서 마주칠 수 있는 세부 결정 사항들. 당장 결정하지 않아도 되지만, 해당 Phase 구현 시 참고.

| # | 항목 | 설명 | 관련 Phase |
|---|------|------|-----------|
| 1 | **미루기 후 과거 수동 알람 처리** | "과거 시각이면 무시"의 구체적 동작 — SW/Capacitor 스케줄에서 아예 제거? 스케줄은 남기되 발송만 안 함? 다음 미루기 시 복원 가능? | Phase 2 |
| 2 | **"하루 종일" overdue 경계 시각** | `dueTime='23:59'`에서 overdue 판정 시각이 23:59:00인지 23:59:59인지. 1분 차이지만 자정 전후 동작에 영향 | Phase 1 |
| 3 | **반복 할일의 PostponeInfo 구조** | 현재 PostponeInfo는 Memo 레벨. 반복 할일에서 인스턴스 A를 3회 미루고 완료 → 인스턴스 B 생성 시 count 리셋? 또는 Memo 레벨 누적? maxAllowed 적용 범위도 연관 | Phase 2~3 |
| 4 | **할일 카드의 다중 알림 표시** | 상기 2개 + 알람 1개 + 자동알람 등 여러 알림이 설정된 경우 카드에 어떻게 표시? 아이콘만? 목록? 가장 가까운 것만? | Phase 1~2 |
| 5 | **동일 기한 할일 정렬 기준** | 같은 dueDate+dueTime인 할일이 여러 개일 때 정렬 tiebreaker — 생성순? 제목 가나다순? 우선순위(향후)? | Phase 1 |
| 6 | **할일 + 체크리스트 완료 의미** | 할일에 체크리스트가 포함된 경우, 체크리스트 전체 완료 ≠ 할일 완료. 체크리스트 진행률 표시 여부, 할일 완료 시 미완료 체크리스트 경고 여부 | Phase 1 |
| 7 | **반복 할일 overdue 다중 인스턴스 뷰 표시** | overdue 인스턴스가 3~4개 쌓인 경우 할일 뷰에서 각각 별도 카드? 그룹핑? "N건 overdue" 요약? | Phase 3 |
| 8 | **완료 Undo 토스트 지속 및 네비게이션** | 3초 Undo 토스트 도중 페이지 이동 시 토스트 유지? 소멸? 다른 할일 완료 시 기존 토스트 교체? | Phase 2 |
| 9 | **기한 없는 할일의 자동알람 처리** | autoAlertBefore는 기한 기준이므로 기한 없는 할일에 적용 불가. UI에서 useGlobalAutoAlert 토글을 숨길지, 보여주되 "기한 설정 시 적용" 안내를 할지 | Phase 2 |
...
| JIT "활성 1개만" | 7.3에서 "현재 활성 1개뿐"이었으나 overdue 누적 결정과 충돌 | overdue 미처리 시 다음 인스턴스도 생성 허용으로 수정 (시나리오 6 추가) |
| 상기 알림 내용 예시 | 3.1.1, 3.1.3에서 요약 형태 메시지("N건")로 되어 있었으나 개별 발송 결정과 충돌 | 개별 알림 형태로 수정 |
| memoType 'task' 주석 | 4.1에서 "기존 'task' → 'todo' 명확화" 주석이 사용 이력 없음과 모순 | "'todo' 신규 추가" 주석으로 정리 |
