...
2. [ ] **콜백과 로그인 시작점의 open redirect 여지를 제거** — 피싱 판정 최우선 후보 방어
   - [ ] `src/routes/auth/callback/+page.svelte`: `returnTo`를 same-origin path-only로 정규화하고 `/login`, 프로토콜 상대 URL, 외부 origin을 모두 `/`로 강등한다
...
3. [ ] **사용자 입력 URL과 외부 열기 흐름을 명시적으로 통제** — social engineering 오인 방지
   - [ ] `src/lib/utils/shareReceiver.ts`: `http`/`https` 외 스킴(`javascript:`, `data:`, `file:` 등)을 저장 전 제거하거나 무효화한다
   - [ ] `src/routes/share/+page.svelte`: 공유받은 URL은 신뢰 배지처럼 보이지 않게 단순 텍스트로 표시하고, 저장 전 검증 실패를 사용자에게 드러내도록 정리한다
   - [ ] `src/lib/stores/notifications.svelte.ts`, `src/lib/components/memo/TodayReminders.svelte`: `window.open()` 경로를 공통 검증 함수로 모으고, 허용 스킴이 아닌 경우 차단한다
   - [ ] `src/service-worker.ts`: `notificationclick`의 `externalUrl` 처리도 동일 기준을 따르도록 맞춰 arbitrary off-origin jump를 줄인다
