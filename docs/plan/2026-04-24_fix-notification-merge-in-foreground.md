...
1. ☐ **공통 병합 로직을 새 유틸 파일로 분리** — SW/메인 스레드가 같은 규칙을 공유
   - ☐ `src/lib/utils/notificationMerge.ts` (신규): `buildMergedBody(titles: string[], maxItems = 4): string` 구현 — 상위 N개 `• {title}` + `외 {rest}건` 축약
   - ☐ `src/lib/utils/notificationMerge.ts` (신규): `buildMergedTitle(count: number): string` 구현 — `"{count}개의 메모 알림"` 반환, 기존 SW 제목과 동일 문구 유지
   - ☐ `src/lib/utils/notificationMerge.ts` (신규): 순수 함수 + import 없음(SW 컨텍스트에서도 로드 가능하도록 $lib 의존 배제)

### Phase 2: Service Worker 병합 본문 개선

2. ☐ **SW `showMergedNotification` 본문을 축약 형태로 교체**
   - ☐ `src/service-worker.ts:199-223` `showMergedNotification`: `titles` 생성 로직을 `buildMergedBody(reminders.map(r => r.title))` 호출로 교체
   - ☐ `src/service-worker.ts`: 제목을 `buildMergedTitle(reminders.length)` 로 통일 (현재 하드코딩과 동일 문구 유지)
   - ☐ `src/service-worker.ts`: SW는 번들러가 유틸을 bundle 해주는지 확인 — 안 되면 `notificationMerge.ts` 내부 함수를 SW 파일 상단에 **복사**(중복 주석 표기)

### Phase 3: 메인 스레드 체크 루프에 병합 도입

3. ☐ **`checkAndTriggerReminders` 를 "수집 → 그룹핑 → 발송"으로 재구성**
   - ☐ `src/lib/stores/notifications.svelte.ts:249-331` `checkAndTriggerReminders`: 루프 내부에서 즉시 `showNotification(memo)` 호출하던 부분을 제거하고, `toFire: Memo[]` 로 수집만 한다
   - ☐ `src/lib/stores/notifications.svelte.ts`: `toFire` 길이가 0 이면 return, 1 이면 기존 `showNotification(memo)` 1회, 2 이상이면 새 `showMergedForeground(memos, currentTime)` 호출
   - ☐ `src/lib/stores/notifications.svelte.ts`: `lastNotifiedMap` / `once → enabled=false` 업데이트는 **발송 전에** 일괄 수행해 동시 분 재진입에 대한 중복 발송을 방지

4. ☐ **`showMergedForeground` 신설** — foreground 경로 병합 알림 발송기
   - ☐ `src/lib/stores/notifications.svelte.ts`: `showMergedForeground(memos: Memo[], time: string)` 추가 — `registration.showNotification(buildMergedTitle(memos.length), { body: buildMergedBody(memos.map(m => m.title)), tag: memo-batch-${time}, data: { memoIds, url: '/', type: 'merged', time }, requireInteraction: true })`
   - ☐ `src/lib/stores/notifications.svelte.ts`: SW ready 분기 실패 시 fallback 으로 `new Notification(...)` 1회만 발송 (각 memo 반복 금지)
   - ☐ `src/lib/stores/notifications.svelte.ts`: 각 memo 에 대해 `notificationHistoryStore.addRecord({ ..., status: 'success' })` 를 반복 호출해 이력을 개별 기록

5. ☐ **snoozed reminder 병합은 범위 밖임을 본 파일에 명시**
   - ☐ `src/lib/stores/notifications.svelte.ts`: snooze 재발화 경로(`snoozedReminders.forEach(...) showNotification(memo, true)`)는 스누즈 해제 시점이 제각각이라 병합 대상이 아님을 코드 주석 1줄로 남긴다
...
6. ☐ **루프 필터를 `reminders[]` 기반으로 수정** — 다중 reminder 시대의 병합 누락 방지
   ...
   - ☐ `src/lib/stores/notifications.svelte.ts`: 이하 `memo.reminder.type/days/date` 참조를 `active.type/active.days/active.date` 로 치환
   - ☐ `src/lib/stores/notifications.svelte.ts`: `once → enabled=false` 전환 시 `memosStore.update()` 가 해당 `active.id` 만 끄도록 `memosStore.updateReminderEnabled(memo.id, active.id, false)` 사용 — 전체 reminders 뭉뚱그려 disable 하지 않기

### Phase 5: 클릭 동작 검증

7. ☐ **foreground 병합 알림 클릭 시 라우팅 확인**
   - ☐ `src/service-worker.ts:509-559` `notificationclick` 핸들러: `data.type === 'merged'` 분기에 foreground 경로에서 온 tag 도 포함되는지 확인(동일 tag 규칙이므로 자연 매칭) — 아니면 주석으로 명시
   - ☐ `src/lib/stores/notifications.svelte.ts`: native `new Notification()` fallback 분기의 `onclick` 에서도 `window.focus()` 후 `location.href = '/'` 로 홈 이동

### Phase 6: 수동/통합 테스트

8. ☐ **foreground 병합 시나리오 수동 검증**
   - ☐ `docs/plan/2026-04-24_fix-notification-merge-in-foreground.md`: 검증 절차를 본 파일 "검증" 섹션에 기록 — 기본 알림 09:00 으로 설정한 메모 13개 생성 → 탭을 열어 둔 상태에서 09:00 도달 → 알림 1건이 뜨고 제목 `"13개의 메모 알림"`, 본문 상위 4건 + `외 9건` 형태인지
...
> T1~T5 해당 없음: 본 plan 은 Python 코드를 수정하지 않는다. 모든 변경은 `src/service-worker.ts`, `src/lib/stores/notifications.svelte.ts`, `src/lib/utils/notificationMerge.ts` (TypeScript) 한정이다. Node 단위 테스트 인프라(Vitest 등)가 이 프로젝트에 선택적이므로 Phase 6 수동 검증으로 대체한다.
